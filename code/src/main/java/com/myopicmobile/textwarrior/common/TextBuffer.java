...
public class TextBuffer implements CharSequence {
    ...
    public TextBuffer() {
        ...
        _contents[MIN_GAP_SIZE] = BaseLanguage.EOF;
        _allocMultiplier = 1;
        _gapStartIndex = 0;
    ...
    private int findCharOffset(int targetLine, int startLine, int startOffset) {
        ...
        while ((workingLine < targetLine) && (offset < _contents.length)) {
            if (_contents[offset] == BaseLanguage.NEWLINE) {
                ++workingLine;
            }
            ++offset;
    ...
    synchronized public int getLineSize(int lineNumber) {
        ...
        if (pos != -1) {
            ...
            while (_contents[pos] != BaseLanguage.NEWLINE &&
                    _contents[pos] != BaseLanguage.EOF) {
                ++lineLength;
                ++pos;
    ...
    public synchronized void delete(int charOffset, int totalChars, long timestamp,
        ...
        for (int i = 0; i < totalChars; ++i) {
            --_gapStartIndex;
            if (_contents[_gapStartIndex] == BaseLanguage.NEWLINE) {
                --_lineCount;
            }
        }
    ...
    private int countNewlines(int start, int totalChars) {
        ...
        for (int i = start; i < (start + totalChars); ++i) {
            if (_contents[i] == BaseLanguage.NEWLINE) {
                ++newlines;
            }
        }
    ...
    protected void initGap(int contentsLength) {
        ...
        _contents[toPosition--] = BaseLanguage.EOF; // mark end of file
    ...
    public void clearSpans() {
        ...
        _spans.add(new Pair(0, BaseLexer.NORMAL));
    }
