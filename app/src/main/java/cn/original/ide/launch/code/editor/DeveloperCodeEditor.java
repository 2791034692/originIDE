...
public class DeveloperCodeEditor extends FreeScrollingTextField {
    ...
    public DeveloperCodeEditor(Context context) {
        this(context, null);
    }

    public DeveloperCodeEditor(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        mContext = context;
        init();
    }

    public View layoutHelpView;

    private void init() {
        ((Activity) getContext()).findViewById(R.id.editor_imageView_layoutHelp);
        ...
        if (getLexer() != null) setLanguage(LanguageJava.getInstance());
        ...
        setOperatorColor(Color.parseColor("#666666"));
    }
    ...
    public void addEvent(String pkg, String[] names) {
        LanguageJava lang = (LanguageJava) getLanguage();
    ...
    public void setNumberColor(int color) {
        getColorScheme().setColor(ColorScheme.Colorable.NUMBER, color);
    }
    ...
    public void open(String filename) {
        path = filename;
        layoutHelpView.setVisibility(View.GONE);
        if (filename == null) {
            setText("未打开文件");
            return;
        }
        if (filename.endsWith(".java")) {
            ...
            if (filename.equals("res/layout/")) {
                layoutHelpView.setVisibility(View.VISIBLE);
            }
